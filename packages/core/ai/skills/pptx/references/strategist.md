...
> 4. Search for each concept's filename in the chosen library: `ls /skill/templates/icons/<chosen-library>/ | grep <keyword>`
...
**When recommending C** — use the host-native path whenever it is available; the manifest/manual path is only a recovery path when the runtime has no image tool:

| Mode | Trigger | Mechanism |
|---|---|---|
| **Host Tool** | Built-in `pptx` default when host has a native image tool (Codex / Antigravity / Claude Code / app image tool / similar) — auto-selected, no user prompting needed | Host-native generation |
| **Offline Manual** | Host has no native image tool available | Prompts written to `images/image_prompts.json`; user generates externally and places files in `project/images/` |

Selection is automatic in Step 5 (Host Tool → Manual). Detailed contract: [`image-generator.md`](./image-generator.md) §7.
...
After the candidates, proceed directly to user confirmation. Do not point to local comparison PNGs; the built-in skill no longer bundles provider-generated reference galleries.
