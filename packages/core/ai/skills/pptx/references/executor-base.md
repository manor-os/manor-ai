...
```bash
ls /skill/templates/icons/chunk-filled/ | grep home
ls /skill/templates/icons/tabler-filled/ | grep home
ls /skill/templates/icons/tabler-outline/ | grep chart
ls /skill/templates/icons/phosphor-duotone/ | grep house
ls /skill/templates/icons/simple-icons/ | grep github
```
...
**Plain presenter notes**: write only what a presenter should say. No visible markers, no labeled meta-lines, no enumerated key-point lists, no duration annotations.

**Per-page structure**: `# <number>_<page_title>` heading, pages separated by `---`. Body is 2–5 natural sentences carrying the page's core message. Page-to-page transitions live inside the opening sentence as natural prose ("接下来……" / "Having framed X, let's turn to Y") — no bracketed `[过渡]` / `[Transition]` tags.
...
- Leaving any bracketed stage marker (`[过渡]` / `[Transition]` / `[Pause]` / `[Data]` / `[Scan Room]` / `[Interactive]` / `[Benchmark]` etc.) in the text.
