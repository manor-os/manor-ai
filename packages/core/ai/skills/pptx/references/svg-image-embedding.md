...
|--------|---------|-------------------|
| **Pending** | Acquisition needed (`Acquire Via: ai`); not yet attempted | Image Acquisition Phase (Step 5) consumes this; must not remain after Step 5 |
...
2. Image Acquisition (Step 5):
   - Pending + ai  → Image_Generator writes image_prompts.json, uses Manor `generate_file(kind="image")`, imports via `import_system_image.py` → Generated
