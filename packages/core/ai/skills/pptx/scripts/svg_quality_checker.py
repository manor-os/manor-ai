...
class SVGQualityChecker:
    ...
    def _check_text_contrast(self, content: str, result: Dict):
        ...
        The PPTX converter preserves explicit fills. A white card with near-white
        text looks acceptable in loose SVG generation but becomes unreadable in
        PowerPoint, so this checker walks the SVG document order and compares text
        ...
        viewbox = self._parse_viewbox_tuple(root.get("viewBox") or "")
        rects = []
        images = []
        texts = []
        order = 0

        def walk(elem, inherited_style):
            nonlocal order
            order += 1
            local_order = order
            ...
            for key in ("fill", "fill-opacity", "opacity", "font-size", "font-weight"):
                value = elem.get(key)
                if value is not None:
                    style[key] = value

            if tag == "rect":
                color = self._parse_solid_color(style.get("fill", "black"))
                opacity = self._parse_opacity(style.get("opacity"), 1.0) * self._parse_opacity(style.get("fill-opacity"), 1.0)
                if color is not None and opacity >= 0.65:
                    rects.append({
                        "order": local_order,
                ...
                color = self._parse_solid_color(style.get("fill", "black"))
                opacity = self._parse_opacity(style.get("opacity"), 1.0) * self._parse_opacity(style.get("fill-opacity"), 1.0)
                if label and color is not None and opacity >= 0.35:
                    font_size = self._parse_svg_number(style.get("font-size"), 16.0)
                    texts.append({
                        "order": local_order,
        ...
        soft_failures = []
        for text in texts:
            bg = self._find_text_background(text, rects, images, viewbox)
            if not bg:
                continue
            ratio = self._contrast_ratio(text["color"], bg["color"])
            message = (
                f"Low-contrast text \"{self._short_text(text['text'])}\" "
                f"({self._format_color(text['color'])} on {self._format_color(bg['color'])}, "
                f"ratio {ratio:.1f}) near x={text['x']:.0f}, y={text['y']:.0f}; "
                "adjust text/background colors before PPTX export"
            )
            if ratio < 2.4:
        ...
        if hard_failures or soft_failures:
            result['info']['low_contrast_text'] = len(hard_failures) + len(soft_failures)
        for message in hard_failures[:8]:
            result['errors'].append(message)
        if len(hard_failures) > 8:
            result['errors'].append(f"Additional low-contrast text issues: {len(hard_failures) - 8}")
        for message in soft_failures[:8]:
            result['warnings'].append(message)
        if len(soft_failures) > 8:
            result['warnings'].append(f"Additional marginal-contrast text issues: {len(soft_failures) - 8}")

    def _find_text_background(self, text: Dict, rects: List[Dict], images: List[Dict], viewbox: Tuple[float, float, float, float] | None):
        probe_points = [
            (text["x"], text["y"]),
        ...
        candidates = [
            ...
            if rect["order"] < text["order"] and any(self._point_in_box(px, py, rect) for px, py in probe_points)
        ]
        if not candidates and viewbox:
            _, _, vb_w, vb_h = viewbox
            candidates = [
                rect for rect in rects
                if rect["order"] < text["order"] and rect["w"] >= vb_w * 0.85 and rect["h"] >= vb_h * 0.85
            ]
        if not candidates:
            return None

        bg = max(candidates, key=lambda rect: rect["order"])
        image_over_bg = any(
            image["order"] > bg["order"]
            and image["order"] < text["order"]
            and any(self._point_in_box(px, py, image) for px, py in probe_points)
            for image in images
        )
        if image_over_bg:
            return None
    ...
    @staticmethod
    def _parse_svg_number(value, default: float = 0.0) -> float:
        if value is None:
            return default
    ...
    def _coord(self, elem, attr: str, default: float = 0.0) -> float:
        return self._parse_svg_number(elem.get(attr), default)

    def _text_coord(self, elem, attr: str, default: float = 0.0) -> float:
        value = elem.get(attr)
        if value is None:
            for child in elem.iter():
                if child is elem:
                    continue
                if self._local_name(child.tag) == "tspan" and child.get(attr) is not None:
                    value = child.get(attr)
                    break
        return self._parse_svg_number(value, default)

    @staticmethod
    def _parse_opacity(value, default: float = 1.0) -> float:
        try:
    ...
    @staticmethod
    def _parse_solid_color(value):
        if value is None:
            return None
        raw = str(value).strip().lower()
        if not raw or raw.startswith("url(") or raw == "none" or raw == "currentcolor":
            return None
        if raw in CSS_COLOR_KEYWORDS:
            return CSS_COLOR_KEYWORDS[raw]
        if raw.startswith("#"):
            hexdigits = raw[1:]
            if len(hexdigits) == 3:
                hexdigits = "".join(ch * 2 for ch in hexdigits)
            elif len(hexdigits) == 4:
                alpha = int(hexdigits[3] * 2, 16)
                if alpha < 90:
                    return None
                hexdigits = "".join(ch * 2 for ch in hexdigits[:3])
            elif len(hexdigits) == 8:
                alpha = int(hexdigits[6:8], 16)
                if alpha < 90:
                    return None
                hexdigits = hexdigits[:6]
            if len(hexdigits) == 6:
                try:
                    return tuple(int(hexdigits[i:i + 2], 16) for i in (0, 2, 4))
                except ValueError:
                    return None
        rgb_match = re.match(r"rgb\(\s*([0-9.]+%?)\s*[, ]\s*([0-9.]+%?)\s*[, ]\s*([0-9.]+%?)\s*\)", raw)
        if rgb_match:
            channels = []
            for channel in rgb_match.groups():
                if channel.endswith("%"):
                    channels.append(round(float(channel[:-1]) * 2.55))
                else:
                    channels.append(round(float(channel)))
            return tuple(max(0, min(255, value)) for value in channels)
        return None

    @staticmethod
    def _point_in_box(x: float, y: float, box: Dict) -> bool:
        return box["w"] > 0 and box["h"] > 0 and box["x"] <= x <= box["x"] + box["w"] and box["y"] <= y <= box["y"] + box["h"]

    @staticmethod
    def _parse_viewbox_tuple(viewbox: str):
        parts = [SVGQualityChecker._parse_svg_number(part, 0.0) for part in viewbox.replace(",", " ").split()]
        if len(parts) != 4:
            return None
        return tuple(parts)
    ...
    def _contrast_ratio(self, fg: Tuple[int, int, int], bg: Tuple[int, int, int]) -> float:
        light = max(self._relative_luminance(fg), self._relative_luminance(bg))
        dark = min(self._relative_luminance(fg), self._relative_luminance(bg))
