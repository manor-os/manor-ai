...
> Standalone preset-creation workflow. Output is a brand template package at `/skill/templates/brands/<brand_id>/`. Structurally a brand is a template minus the SVG page roster — Strategist locks the brand's color / typography / logo / voice as truth; Executor designs pages freely under those constraints.
...
| User provides a logo/image asset or verbal brand spec and wants it locked across future projects | Run this workflow |
...
|---|---|---|
| **A** Brand asset | Logo or brand image (SVG/PNG/JPG) | Step 2A — Asset extraction |
...
Read assets directly. This built-in skill does not run document, deck, or URL converters.
...
- `[fact]` — extracted directly (SVG fill HEX)
...
```bash
mkdir -p "/skill/templates/brands/<brand_id>"
```
...
```markdown
## ✅ Brand Saved
- Path: `/skill/templates/brands/<brand_id>/`
- Files: design_spec.md{, logo.<ext>}{, images/}{, illustrations/}{, icons/}
- Fields locked: <list>
- Provenance: <fact / approx / user counts>

How to use in a project:
- Include the brand directory path in your initial Step 3 input — e.g. "做一个 Q4 总结 PPT, 用 /skill/templates/brands/<brand_id>/ 这个品牌"
...
- To edit: modify `templates/brands/<brand_id>/design_spec.md` directly, then re-run `python3 /skill/scripts/register_template.py --kind brand <brand_id>`
```
