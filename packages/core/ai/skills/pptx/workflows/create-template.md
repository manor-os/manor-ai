...
```bash
python3 /skill/scripts/pptx_template_import.py "<reference_template.pptx>"
```
...
```bash
mkdir -p "/skill/templates/layouts/<template_id>"
```

> **Output location**: Global templates go to `/skill/templates/layouts/`; project templates go to `projects/<project>/templates/`
>
...
```bash
ls -la "/skill/templates/layouts/<template_id>"
```

Run SVG validation on the template directory:

```bash
python3 /skill/scripts/svg_quality_checker.py "/skill/templates/layouts/<template_id>" --template-mode --format <canvas_format>
```
...
```bash
python3 /skill/scripts/register_template.py <template_id>
```

Outputs:

- updates `/skill/templates/layouts/layouts_index.json` — the flat `template_id → { summary, keywords }` map
- refreshes the auto-managed Quick Index inside `/skill/templates/layouts/README.md` (the surrounding category sections stay hand-edited)
...
>
> ```bash
> python3 /skill/scripts/register_template.py --rebuild-all
> ```
