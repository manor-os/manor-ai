...
First check whether `/tmp/fonts/OTF/SimplifiedChinese/SourceHanSansSC-Regular.otf` exists. If it exists, use `fpdf2` with the SourceHanSansSC OTF fonts as the preferred path. If `fpdf2` is missing, install it once, then proceed. Do not install multiple PDF or font libraries.
...
Only use ReportLab for CJK text if a verified TrueType-outline `.ttf` font already exists and `pdfmetrics.registerFont(TTFont(...))` succeeds on the first try. Otherwise, use `fpdf2` or an HTML-to-PDF/browser rendering fallback. Do not repeatedly download fonts from the internet. If SourceHanSansSC fonts are missing and no HTML-to-PDF renderer is available, stop and report the missing dependency instead of retrying downloads.
...
| Create Chinese/CJK PDFs | fpdf2 with SourceHanSansSC OTF | Use `/tmp/fonts/OTF/SimplifiedChinese/SourceHanSansSC-Regular.otf`; HTML-to-PDF/browser rendering is fallback |
